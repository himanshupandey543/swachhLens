...
(function () {
  ...
  const CAT_LABEL = { dry: t('dash.catDryPlain'), wet: t('dash.catWetPlain'), ewaste: t('dash.catEwastePlain') };
  const PT_LABEL  = { gate: t('dash.ptGatePlain'), corner: t('dash.ptCornerPlain') };
  ...
  function renderTruck(t) {
    t = t || TRUCK;
    truckLbl.innerHTML = '<b>' + escapeHtml(t.driver || t('gis.truckCrew')) + '</b><span>' +
      escapeHtml(t.plate || '') + ' · ' + escapeHtml(t.ward || '') + ' · ' + (t.speedKmh || 0) + ' km/h</span>';
  }

  /* ---- Render dustbin markers (color-coded status pins) ---- */
  const STATUS = { ok: t('gis.okStatus'), watch: t('gis.watchStatus'), alert: t('gis.alertStatus') };
  function renderBins() {
    binWrap.innerHTML = '';
    BINS.forEach(function (b, i) {
      ...
      el.setAttribute('aria-label', t('gis.binAria', { id: b.id, pct: b.fill, st: STATUS[level] }));
      el.innerHTML =
        '<span class="pin"><b>' + b.fill + '%</b></span>' +
        '<div class="bin-pop">' +
          '<div class="bp-head"><b>' + t('gis.bin') + ' ' + b.id + '</b><span class="bp-status ' + level + '">' + STATUS[level] + '</span></div>' +
          '<div class="bp-bar"><i style="--f:' + b.fill + '%"></i></div>' +
          '<div class="bp-fill"><span>' + t('gis.fillLevel') + '</span><b>' + b.fill + '%</b></div>' +
          '<div class="bp-meta">📍 ' + t('gis.fromRoute', { d: Math.round(distToRoute(b.x, b.y)), t: b.type }) + '</div>' +
        '</div>';
  ...
  function tick(now) {
    ...
    if (state.handoff.arrived) {
      ...
      if (now - state.handoff.arrivedAt > 5000) {
        ...
        dispatchCard.querySelector('.gd-tag').textContent = t('dash.dispatchTag');
      }
    } else {
  ...
  function onArrived() {
    ...
    dispatchCard.querySelector('.gd-tag').textContent = t('gis.arrivedTag');
    toast(t('gis.arrivedToast'));
  }
